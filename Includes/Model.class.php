...
class Model {
    protected $mysqli     = Null;                              //mysqli实例对象
    private   $conf       = array();                  //config配置信息
    public    $sql;                                          //sql语句
    protected $rs;                                      //结果集
    ...
    public function  __construct() {
        $this->conf   = json_decode(file_get_contents(ROOT.'config'.DS.'mysqli.json'),true);
        if ($this->mysqli == Null) {
    ...
    protected function get_query_sql($sql, $limit = null) {
        if (@preg_match("/[0-9]+(,[ ]?[0-9]+)?/is", $limit) && !preg_match("/ ^SELECT|LIMIT [0-9]+(,[ ]?[0-9]+)?$/is", $sql)) {
            $sql .= " LIMIT " . $limit;
        }
        return $sql;
    }
